			...
			link    : function(originalScope, element, attrs, modelCtrl) {
				...
				function getHeaderText() {
					if(isEmpty(modelCtrl.$modelValue)) {
						scope.header = attrs.msHeader || "Seleccione Rangos Etáreos";
						return scope.header;
					}
					...
					if(isMultiple) {
						if(attrs.msSelected) {
							...
							var s = modelCtrl.$modelValue.length === 1 ? '' : 's';
							scope.header = modelCtrl.$modelValue.length + " Rango" + s + " Etáreo" + s + " Seleccionado" + s;
						}
...
angular.module("multiselect.tpl.html", []).run(["$templateCache", function($templateCache) {
	$templateCache.put("multiselect.tpl.html",
			...
			"  <button tabindex=\"{{tabindex}}\" title=\"{{header}}\" type=\"button\" style=\"width:100%;\" class=\"btn btn-primary dropdown-toggle\" data-ng-click=\"toggleSelect()\" data-ng-disabled=\"disabled\" data-ng-class=\"{'error': !valid()}\">\n" +
			"    <div style=\"text-align: left; padding-right: 13px; overflow: hidden; text-overflow: ellipsis; width:100%;\">{{header}}</div><span class=\"caret\" style=\"position:absolute;right:10px;top:14px;\"></span>\n" +
			"  </button>\n" +
			"  <ul class=\"dropdown-menu\" style=\"margin-bottom:30px;padding-left:5px;padding-right:5px;\" data-ng-style=\"ulStyle\">\n" +
			"    <input data-ng-show=\"items.length > filterAfterRows\" ng-model=\"filter\" style=\"padding: 0px 3px;margin-right: 15px; margin-bottom: 4px;\" placeholder=\"Escriba para filtrar opciones\">" +
			"    <li data-stopPropagation=\"true\" data-ng-repeat=\"i in items | filter:filter\">\n" +
			"      <a data-ng-click=\"select($event, i)\" style=\"padding:3px 10px;cursor:pointer;\">\n" +
			"        <i class=\"glyphicon\" data-ng-class=\"{'glyphicon-ok': i.checked, 'empty': !i.checked}\"></i> {{i.label}}</a>\n" +
			"    </li>\n" +
			"  </ul>\n" +
			"</div>");
}]);
